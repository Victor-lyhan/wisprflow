...
in CI failed with ``ModuleNotFoundError: No module named 'flowscribe.audio'``.
...
``src/flowscribe/audio/`` and ``src/flowscribe/dental/data/``. The files were
...
class TestSourceTree:
    ...
    def test_subpackage_is_importable(self, package: str) -> None:
        __import__(f"flowscribe.{package}")
    ...
    def test_subpackage_is_tracked_by_git(self, package: str) -> None:
        ...
        result = subprocess.run(
            ["git", "ls-files", f"src/flowscribe/{package}/"],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
        )
        ...
        assert result.stdout.strip(), f"src/flowscribe/{package}/ is untracked or ignored"

    @pytest.mark.parametrize("relative", REQUIRED_DATA)
    def test_data_file_is_tracked_by_git(self, relative: str) -> None:
        result = subprocess.run(
            ["git", "ls-files", f"src/flowscribe/{relative}"],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            pytest.skip("not a git checkout")
        assert result.stdout.strip(), f"src/flowscribe/{relative} is untracked or ignored"
...
class TestBuiltWheel:
    ...
    def test_wheel_contains_subpackage(self, wheel: Path, package: str) -> None:
        ...
        assert any(name.startswith(f"flowscribe/{package}/") for name in names), (
            f"wheel is missing flowscribe/{package}/"
        )
    ...
    def test_wheel_contains_data_file(self, wheel: Path, relative: str) -> None:
        ...
        with zipfile.ZipFile(wheel) as archive:
            names = archive.namelist()
        assert f"flowscribe/{relative}" in names, f"wheel is missing flowscribe/{relative}"
